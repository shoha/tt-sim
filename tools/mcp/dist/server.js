 ...
 */
function stepTimeoutMs(frames, seconds) {
    const requested = frames !== undefined && frames > 0
        ? frames
...
function stepUntilTimeoutMs(maxFrames) {
    return stepTimeoutMs(maxFrames !== undefined && maxFrames > 0 ? maxFrames : DEFAULT_STEP_UNTIL_MAX_FRAMES, undefined);
}
// ---------------------------------------------------------------------------
// Lifecycle tools
// ---------------------------------------------------------------------------
server.tool("game_launch", "Launch Godot with the validation bridge. Must be called before other game_ tools.", {
    scene: z
        .string()
        .optional()
    ...
    try {
        ...
        return {
            content: [{ type: "text", text: `Launch failed: ${msg}` }],
            isError: true,
        };
    }
});
...
server.tool("game_reload", "Stop and relaunch Godot. Use after making code changes.", {
...
// Query tools
// ---------------------------------------------------------------------------
server.tool("game_screenshot", "Capture a screenshot of the game window. Returns the image.", {}, async () => {
...
server.tool("game_state", "Query current game state: app state, tokens, UI panels, camera, scene tree, console errors, " +
    "and the clock ('frozen' and 'time_scale'). Check 'frozen' first when nothing seems to move: " +
    "a frozen game is indistinguishable from a hung one until you look at it.", {}, async () => {
    const err = requireBridge();
    if (err)
        return { content: [{ type: "text", text: err }], isError: true };
    ...
    if (!result.ok) {
        return {
            content: [{ type: "text", text: `State query failed: ${result.error}` }],
            isError: true,
        };
    }
    const errors = pm.getErrors();
...
// Interaction tools
// ---------------------------------------------------------------------------
server.tool("game_click", "Click at window coordinates.", {
    ...
    return {
        content: [{ type: "text", text: result.ok ? "Clicked." : `Error: ${result.error}` }],
    };
});
server.tool("game_drag", "Drag from one window position to another.", {
    ...
    return {
        content: [{ type: "text", text: result.ok ? "Dragged." : `Error: ${result.error}` }],
    };
});
server.tool("game_key", "Press and release a key, optionally with modifiers.", {
    key: z
        .string()
        .describe("Key name (e.g. 'M', 'Escape', 'Home', 'Space'), or a chord with " +
    ...
    return {
        content: [
            { type: "text", text: result.ok ? `Key '${key}' pressed.` : `Error: ${result.error}` },
        ],
    };
});
server.tool("game_scroll", "Scroll the mouse wheel at a window position.", {
    x: z.number().describe("X coordinate, in window pixels (same space as game_screenshot pixels)"),
    y: z.number().describe("Y coordinate, in window pixels (same space as game_screenshot pixels)"),
    ...
    return {
        content: [{ type: "text", text: result.ok ? "Scrolled." : `Error: ${result.error}` }],
    };
});
server.tool("game_wait", "Wait for a duration (for animations/transitions to settle).", {
    ...
    return {
        content: [
            { type: "text", text: result.ok ? `Waited ${seconds}s.` : `Error: ${result.error}` },
        ],
    };
});
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
server.tool("game_time", "Control game time deterministically. 'freeze' stops the clock (physics stops, process delta " +
    ...
    expression: z
        .string()
        .optional()
        .describe("For 'step_until': GDScript expression evaluated against the current scene each frame."),
    maxFrames: z
        .number()
        .int()
        .optional()
    ...
    switch (action) {
        ...
        case "resume":
            ...
            break;
        case "step":
            result = await pm.bridge.send({ cmd: "step", seconds, frames }, stepTimeoutMs(frames, seconds));
            break;
        case "step_until":
...
// Inspection
// ---------------------------------------------------------------------------
server.tool("game_eval", "Evaluate a GDScript expression against the running game's current scene and return the value. " +
...
server.tool("game_controls", "List Control nodes in the running game with their viewport-space rect and centre point, up " +
...
server.tool("game_click_control", "Click a Control by node name, node path, or exact button text. Prefer this over game_click: " +
...
const StepSchema = z.discriminatedUnion("action", [
    ...
    z.object({
        ...
        key: z
            .string()
            .describe("Key name (e.g. 'M', 'Escape'), or a chord with 'Ctrl'/'Shift'/'Alt' " +
    ...
    z
        ...
        .describe("List Control nodes with their viewport rect, centre, and window_point (window pixels, " +
...
server.tool("game_interact", "Execute a sequence of interactions and return collected screenshots and state. " +
