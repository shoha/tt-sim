...
- **Settings persistence**: Decentralized — each system reads/writes its own section in `user://settings.cfg`
...
- **Threading**: Use `WorkerThreadPool` for one-off jobs; always call `wait_for_task_completion()` after polling. Guard `call_deferred()` from threads against freed nodes
