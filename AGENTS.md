...
| `game_controls` | List every Control with viewport-space rect and centre. Sees dialogs parented to the window root |
| `game_click_control` | Click a Control by name, path, or button text. No coordinate conversion needed |
...
- **`game_controls` returns at most 200 Controls and truncates silently** — the response carries no marker that it was cut off. A control you cannot find may be past the cap rather than absent from the scene
