...
- **Foliage primitive budget** – Imported (`user://`) maps are capped at a fixed total scattered-foliage primitive count (`FoliageBudget.PRIMITIVE_BUDGET`, `utils/foliage_budget.gd`) and thinned proportionally if they exceed it, because a MultiMesh is culled as one AABB so every instance is vertex-processed whenever any part of its species is on screen (superseded by spatial chunking below -- true for every map before chunking landed, now only the worst case at full zoom-out). Applied by `ScatterGlbUtils.process_scatter_instances()`; the player gets one toast from `level_loader.gd`. Built-in `res://` maps never run the scatter pipeline. Measured attribution, the budget's derivation, known dead ends, and the measurement procedure are all in `docs/PERFORMANCE.md` – read it before attempting any rendering optimisation
