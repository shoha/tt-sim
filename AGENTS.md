...
| `game_controls` | List Controls (max 200, with a `truncated` flag) with viewport-space rect and centre. Sees dialogs parented to the window root. **The reported centre is canvas space, which is NOT what `game_click` takes** — feed it to `game_click_control` instead, or convert it first |
