...
- **Level list flow** – `TitleScreen`'s `LevelGrid.provider` (default `LevelManager.get_saved_levels`) supplies the `level_info` Dictionaries behind every card. Picking one and pressing Host/Join/Play Solo emits `host_game_requested`/`play_solo_requested(level_info)`; `Root` loads it with `LevelManager.load_level()` into `_pending_level_data` before `change_state(LOBBY_HOST/PLAYING)`. The host lobby's Change button and the pause menu's Change Level both open the same `LevelPickerDialog`: the lobby (`_on_lobby_level_change_requested`) just replaces `_pending_level_data` before Start; the pause menu (`_on_pause_change_level_requested`) reloads the level in place through `GameplayMenuController.request_level_change(on_ready)` (which prompts to discard unsaved Visuals-drawer edits first) and `Root.request_level_change(level_info)`, broadcasting to clients if hosting. `LevelManager.save_thumbnail(level_data, image)` writes a 320x180 PNG beside `level.json` (fit via `LevelThumbnail.fit()`); `rename_level()` and `duplicate_level()` back the card's overflow menu
...
- **Water settings** – Per-level water tuning is a typed `WaterSettings` (`resources/water_settings.gd`; keys are `shaders/water.gdshader` uniform names, colours serialised as hex strings), stored on `LevelData.water` under the `water_overrides` JSON key next to the legacy `water_style` (written as the matching Look, or `custom`; a level without `water_overrides` is seeded from its style). `WaterPresets` holds three independent groups (`LOOKS`, `PALETTES`, `MOTIONS`) that partition the keys; the Water pane derives each tile row by value matching (`WaterSettings.matching_*`). Live apply is `WaterGlbUtils.apply_water_settings()` via `LevelPlayController.apply_water_settings()`, broadcast under `water_overrides`. A flow map baked by terrain-paint arrives as the water plane's `emission_texture`; `WaterGlbUtils.process_water_meshes()` sets it as the shared material's `water_flow_map` and flags the largest-footprint plane with the `water_flow_present` instance uniform (spec: `docs/superpowers/specs/2026-09-23-water-flow-design.md`)
...
- **New menu screen**: open with a `MenuHeader` (`setup(title, caption, closable)`, sentence case), build actions with `UiActions.primary()` / `UiActions.secondary()` — exactly one accent primary per screen, everything else `Secondary` with an icon — put the footer in an `HBoxContainer` with `alignment = ALIGNMENT_END` (secondary actions first, primary last), and stagger the body in with `UiMotion.stagger_in(UiMotion.visible_children(box), self)`. `Success`/`Danger` fills belong only to `ConfirmationDialogUI`'s confirm button. See `docs/THEME_GUIDE.md` UI Primitives and `docs/UI_SYSTEMS.md` Menu Design Language
...
- **New title action**: add it in `TitleScreen._build_left_column()` via `UiActions.secondary()` (or `UiActions.primary()` for a tall, captioned action like Host/Join)
...
- **New lighting preset**: Add to `EnvironmentPresets.PRESETS` in `utils/environment_presets.gd`, and list it in `EnvironmentPresets.PRESET_GROUPS` — the Look picker is built from the groups, and `test_environment_presets_groups.gd` fails if a preset is missing from a group
- **New sky preset**: Add a manifest entry to `tools/skies_manifest.json`, run `tools/curate_skies.gd` (headless editor) then `godot --headless --import --path .`, paste the printed entry into `EnvironmentPresets.SKY_PRESETS`, add a tile to `SkyPane.SKY_TILES`, add the licence row in `THIRD_PARTY_LICENSES.md`; a sky without a panorama is a gradient-only entry
...
| `game_click` | Click at (x, y) window coords (same space as `game_screenshot` pixels, and what `game_controls`' `window_point` reports). Optional `button`: "left" (default), "right", "middle" |
...
| `game_controls` | List Controls (max 200, with a `truncated` flag) with viewport-space rect and centre. Sees dialogs parented to the window root. **The reported `center` is canvas space, which is NOT what `game_click` takes** — each entry also carries a `window_point`, the same centre converted to window pixels, which `game_click` *can* take directly; or use `game_click_control` instead |
| `game_click_control` | Click a Control by name, path, or button text. The bridge converts canvas space to window space, so the caller does no coordinate math — prefer this over feeding `game_controls`' `window_point` to `game_click` by hand |
...
- **Clicks land on nothing / buttons do not react**: almost always a coordinate-space mismatch, not a broken bridge. `game_state` reports `viewport.window_size`, `viewport.viewport_size` and `viewport.hovered_control` — check `hovered_control` after a click to confirm what you actually hit before concluding anything else. `game_click_control` avoids this problem — the bridge converts the Control's canvas-space centre into window space with `get_viewport().get_final_transform()` before injecting, so the caller does no coordinate math — and is the right default; reach for `game_click` with raw coordinates only for 3D world clicks, or with a `game_controls` entry's `window_point` (the same conversion, already applied) when a raw click is still needed. (This conversion was missing until it was measured: `click_control` used to inject the unconverted centre, miss the Control, and still answer `{"ok": true}`, because it only ever checked that it had *found* the Control. Verify with `hovered_control` if a named click ever looks wrong.)
- **Controls on an overlay `CanvasLayer` are clickable, including the F3 debug render panel.** This project's code and notes used to say the `DebugRenderToggles` checkbox panel "cannot be driven by the validation bridge's injected mouse clicks" and that the cause was "still unexplained". Both claims were wrong. `game_click_control` on `Baked foliage AO` flips `toggle_baked_foliage_ao` reliably (measured 8/8 at a 1278x1360 window over a 1920x2043 viewport, frozen and running), and `hovered_control` afterwards names the `CheckBox` itself. What actually failed was a raw `game_click` sent at the centre `game_controls` reported: canvas `(163, 679)` as a window coordinate lands at canvas `(245, 1020)`, which `hovered_control` shows as the `SubViewportContainer` — the 3D map — while the command still answers `{"ok": true}`. The same click at the converted window point `(108.5, 452)` toggles the checkbox. Three things that look like plausible causes are not: the perf-overlay container's `MOUSE_FILTER_IGNORE` (Godot skips only the IGNORE node itself and still hit-tests its children), the overlay `CanvasLayer`'s transform (identity), and the full-screen `TransitionOverlay`/`ToastContainer` Controls sitting on higher layers (both `MOUSE_FILTER_IGNORE`). The digit-key shortcuts (1-9, 0) remain the faster way to sweep all ten switches, but they are a convenience now, not a workaround
