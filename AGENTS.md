...
- **In-game editing** – `LevelEditPanel` (extends `DrawerContainer`, right edge) provides real-time editing during gameplay (map, lighting, environment, post-processing, weather). `GameplayMenuController` routes changes to `LevelPlayController`. Cancel reverts; save persists to disk.
  ...
  - Sun section: offers direction (numeric Azimuth/Elevation fields plus the interactive `SunGizmoTool` ground-compass), color, energy, and shadow enable/softness/darkness, with Time of Day acting as a generator that can (re)populate direction/color/energy from an hour rather than driving them directly
  ...
  - Dirty state and prompts: the panel tracks unsaved edits (`is_dirty()`/`_mark_dirty()`/`mark_clean()`) — a dirty drawer shows a tab badge and tooltip, and vetoes a tab-click or Escape close with a "Discard changes" / "Keep editing" prompt via `request_close()`; Save clears the flag but leaves the drawer open instead of closing it
  ...
  - Extracted sub-components: `LevelEditOverrideRows` and `LevelEditSunSection` are the panel's extracted sub-components (override-row indicators/reset, and the Sun section, respectively), split out to stay under the project's max-file-lines lint budget
