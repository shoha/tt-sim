...
func has_cached(pack_id: String, asset_id: String, variant_id: String, file_type: String = "model") -> bool:
...
func store_asset(pack_id: String, asset_id: String, variant_id: String, data: PackedByteArray, file_type: String = "model") -> String:
	var key = _make_key(pack_id, asset_id, variant_id, file_type)
...
func register_cached_file(pack_id: String, asset_id: String, variant_id: String, file_path: String, file_type: String = "model") -> void:
...
func get_expected_cache_path(pack_id: String, asset_id: String, variant_id: String, file_type: String = "model") -> String:
	return _get_cache_path(pack_id, asset_id, variant_id, file_type)


## Remove a specific asset from cache
func remove_cached(pack_id: String, asset_id: String, variant_id: String, file_type: String = "model") -> void:
	var key = _make_key(pack_id, asset_id, variant_id, file_type)
...
func _get_cache_path(pack_id: String, asset_id: String, variant_id: String, file_type: String) -> String:
...
func _evict_if_needed(incoming_size: int) -> void:
	...
	for item in entries:
		...
		print("AssetCacheManager: Evicted %s (%.1f MB freed)" % [key, entry.size_bytes / 1024.0 / 1024.0])
...
func _load_cache_index() -> void:
	...
	print("AssetCacheManager: Loaded cache index with %d entries (%.1f MB)" % [
		_cache_index.size(),
		get_cache_size_mb()
	])
