...
signal asset_failed(
	request_id: String, pack_id: String, asset_id: String, variant_id: String, error: String
)
...
class ResolutionRequest:
	...
	func _init(
		p_id: String, a_id: String, v_id: String, f_type: String = "model", prio: int = 100
	) -> void:
...
func resolve_model_sync(
	pack_id: String, asset_id: String, variant_id: String = "default"
) -> String:
...
func is_available_locally(
	pack_id: String, asset_id: String, variant_id: String = "default", file_type: String = "model"
) -> bool:
...
func _resolve_async(
	pack_id: String, asset_id: String, variant_id: String, file_type: String, priority: int
) -> String:
	# Create request
	...
	if local_path != "":
		...
		call_deferred(
			"_emit_resolved", request.request_id, pack_id, asset_id, variant_id, local_path
		)
	...
	if cached_path != "":
		call_deferred(
			"_emit_resolved", request.request_id, pack_id, asset_id, variant_id, cached_path
		)
		return request.request_id
...
func _on_http_download_progress(
...
func _complete_request(request_id: String, local_path: String, error: String) -> void:
	...
	if local_path != "":
		asset_resolved.emit(
			request_id, request.pack_id, request.asset_id, request.variant_id, local_path
		)
	else:
