...
func _are_hints_equal(new_hints: Array) -> bool:
	...
	for i in range(new_hints.size()):
		if new_hints[i].key != _current_hints[i].key or new_hints[i].action != _current_hints[i].action:
			return false
...
func _animate_in() -> void:
	...
	_tween.tween_property($MarginContainer, "modulate:a", 1.0, 0.2)
...
func _animate_out() -> void:
	...
	_tween.set_trans(Tween.TRANS_CUBIC)
	_tween.tween_property($MarginContainer, "modulate:a", 0.0, 0.15)
