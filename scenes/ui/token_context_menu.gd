...
# Predefined damage amounts for quick actions
@export var quick_damage_amounts: Array[int] = [1, 5, 10, 20]
...
func _on_damage_button_pressed(amount: int) -> void:
	if target_token:
		damage_requested.emit(amount)
		_update_menu_content()

func _on_custom_damage_requested() -> void:
	var input_field = $MenuPanel/VBoxContainer/CustomDamageContainer/CustomDamageInput
	var amount = int(input_field.text)
	if amount > 0:
		_on_damage_button_pressed(amount)
		input_field.clear()

func _on_heal_button_pressed(amount: int) -> void:
	if target_token:
		heal_requested.emit(amount)
		_update_menu_content()
